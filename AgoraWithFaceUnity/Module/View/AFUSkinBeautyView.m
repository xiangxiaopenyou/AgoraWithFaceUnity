...
static NSString * const kAFUSkinBeautyCellIdentifier = @"AFUSkinBeautyCell";

@interface AFUSkinBeautyView ()<UICollectionViewDelegate, UICollectionViewDataSource>

@property (nonatomic, strong) FUSlider *slider;
@property (nonatomic, strong) UICollectionView *skinBeautyCollection;
/// 恢复按钮
...
@implementation AFUSkinBeautyView
...
- (instancetype)initWithFrame:(CGRect)frame items:(NSArray<AFUBeautyItem *> *)items {
    ...
    if (self) {
        ...
        [self addSubview:self.skinBeautyCollection];
        [self.skinBeautyCollection mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (void)recoverAction {
    ...
    UIAlertAction *certainAction = [UIAlertAction actionWithTitle:@"确定" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(skinBeautyViewShouldRecoverEffects)]) {
            [self.delegate skinBeautyViewShouldRecoverEffects];
            [self changeSliderValue];
            [self.skinBeautyCollection reloadData];
...
- (void)sliderValueChanged:(FUSlider *)slider {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(skinBeautyViewDidChangeValue)]) {
        [self.delegate skinBeautyViewDidChangeValue];
    }
}
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    AFUBeautyCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:kAFUSkinBeautyCellIdentifier forIndexPath:indexPath];
...
- (UICollectionView *)skinBeautyCollection {
    if (!_skinBeautyCollection) {
        ...
        _skinBeautyCollection = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        _skinBeautyCollection.backgroundColor = [UIColor clearColor];
        _skinBeautyCollection.showsHorizontalScrollIndicator = NO;
        _skinBeautyCollection.delegate = self;
        _skinBeautyCollection.dataSource = self;
        [_skinBeautyCollection registerClass:[AFUBeautyCell class] forCellWithReuseIdentifier:kAFUSkinBeautyCellIdentifier];
    }
    return _skinBeautyCollection;
}
